...
class UTSOneByOneDataset(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, tsData: TSData, phase:str, window_size: int, pred_len: int) -> None:
        super().__init__()
        ...
        if phase == "train":
            ...
            for i in range(self.sample_num):
                ...
                Y[i, :] = torch.from_numpy(np.array(
                    tsData.train[i + self.window_size: i + self.window_size + pred_len]
                ))
            ...
            for i in range(self.sample_num):
                ...
                Y[i, :] = torch.from_numpy(np.array(
                    tsData.valid[i + self.window_size: i + self.window_size + pred_len]
                ))
            ...
            for i in range(self.sample_num):
                ...
                Y[i, :] = torch.from_numpy(np.array(
                    tsData.test[i + self.window_size: i + self.window_size + pred_len]
                ))
...
class UTSAllInOneDataset(torch.utils.data.Dataset):
    ...
    The Dateset for one by one training and testing. 
    '''
    def __init__(self, datas: Dict[str, TSData], phase:str, window_size: int, pred_len: int) -> None:
        super().__init__()
        self.window_size = window_size
        self.sample_num = 0
        ...
        if phase == "train":
            for _, i in datas.items():
                ll, = i.train.shape
                self.sample_num += max(ll - self.window_size - pred_len + 1, 0)
            ...
            for _, i in datas.items():
                ll, = i.train.shape
                ll = max(ll - self.window_size - pred_len + 1, 0)
                for j in range(ll):
                    X[cnt, :] = torch.from_numpy(i.train[j: j + self.window_size])
                    Y[cnt, :] = torch.from_numpy(np.array(
                        i.train[j + self.window_size: j + self.window_size + pred_len]
                    ))
                    cnt += 1
            ...
            for _, i in datas.items():
                ...
                for j in range(ll):
                    ...
                    Y[cnt, :] = torch.from_numpy(np.array(
                        i.valid[j + self.window_size: j + self.window_size + pred_len]
                    ))
                    cnt += 1
